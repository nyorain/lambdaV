...
\begin{prooftree}
	\AxiomC{$e_i \conv e_i'$}
	\UnaryInfC{$(\dots e_i \dots) \conv (\dots e_i' \dots)$}
\end{prooftree}
...
\begin{prooftree}
	...
	\AxiomC{$\varnothing, \varnothing \vdash e_i : \tau_i$}
	\BinaryInfC{$R, ((e_1 \dots e_n)) \vdash builtin : \tau$}
\end{prooftree}
...
\fbox{\begin{minipage}{0.9\textwidth}
	...
	\textbf{then} $eval_S(init_S(O.consts, O.code), O.ido) = o$.
	\end{flushleft}
\end{minipage}}
\end{center}
\medskip
...
\fbox{\begin{minipage}{1\textwidth}
	...
	\textbf{then} $O.type = \tau$ and \\
	if $O.ido \neq 0$, it has type $idtype(\tau)$, and \\
	$eval_S(init_S(O.consts, O.code), O.ido) = o$.
\end{minipage}}
\end{center}
\medskip
...
% We could easily formalize this proof given formalizations of 
% $eval_S$ and $init_S$.
...
SPIR-V counterparts 
% (see the example for the operational semantics of OpFAdd,
